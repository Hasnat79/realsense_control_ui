...
Then reload your shell, initialise the Go module (first time only), and run:

```bash
source ~/.bashrc

# First time only — creates go.mod in the project directory
# The module path is just a local identifier, not a URL
...
### `go: cannot find main module` / `module path must be specified`
Run these in the project directory (first time only):
```bash
go mod init realsense-capture
go mod tidy
```
The module path (`realsense-capture`) is just a local identifier — it doesn't
need to be a URL or match anything online.
